...
function ProductCard({
  ...
  return (
    <div className="group rounded-3xl border border-black/10 dark:border-white/10 bg-white dark:bg-gray-800/50 p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
      <div className="mb-4 aspect-square overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-5xl">
        👕
      </div>
      <div className="mb-2 flex flex-wrap gap-1">
        {product.idcategories?.map((catId) => (
          <span key={catId} className="inline-block rounded-full bg-gray-100 dark:bg-white/5 px-2 py-0.5 text-xs font-medium text-pink-600 dark:text-pink-300">
      ...
      <p className={`mt-1 text-xs font-medium ${product.stock > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-500"}`}>
        {product.stock > 0 ? `${product.stock} disponibles` : "Sin stock"}
      </p>
      <div className="mt-3 flex items-center justify-between">
        <span className="text-lg font-bold text-pink-600">${product.price.toLocaleString("es-MX")}</span>
        <button
          onClick={onAgregar}
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${
      ...
      <section className="relative h-[90.7vh] min-h-96 overflow-hidden">
        <Image src={heroImage} alt="Imagen de moda" fill className="object-cover object-center opacity-30" priority />
        ...
        <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col justify-center px-4 py-16 text-center text-white sm:px-6 lg:px-8">
          <p className="mb-4 text-sm uppercase tracking-[0.35em] text-pink-200 sm:text-base">Colección exclusiva</p>
          <h1 className="text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">Ropa con estilo y actitud</h1>
          ...
          <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <a href="#productos" className="rounded-md bg-pink-500 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-pink-500/30 transition hover:bg-pink-400">
              Ver Productos
            </a>
            {/* ── Botón Contáctanos ── */}
            <a href="#contacto" className="rounded-md border border-white/40 bg-white/10 px-6 py-3 text-sm font-semibold text-white backdrop-blur-sm transition hover:bg-white/20">
              Contáctanos
            </a>
          </div>
        </div>
                ...
                <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                  {products.map((p) => (
                    <ProductCard
                      ...
                      onAgregar={() =>
                        addToCart({ id: p.id, name: p.name, price: p.price, description: p.description })
                      }
                    />
                  ))}
                </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="overflow-hidden rounded-4xl bg-white dark:bg-slate-900/90 p-8 shadow-2xl ring-1 ring-gray-200 dark:ring-white/10 sm:p-12">
            <div className="grid gap-12 lg:grid-cols-[1.3fr_0.9fr] lg:items-center">
              <div>
                <p className="text-sm uppercase tracking-[0.35em] text-pink-600 dark:text-pink-300">Sobre Nosotros</p>
                <h2 className="mt-4 text-3xl font-bold tracking-tight sm:text-4xl">Moda con propósito y estilo auténtico</h2>
              ...
              <div className="space-y-6 rounded-3xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 p-6 shadow-sm">
                {[
                  ...
                  { titulo: "Atención personalizada", desc: "Nuestro equipo está listo para ayudarte a elegir lo que mejor te queda." },
                ].map(({ titulo, desc }) => (
                  <div key={titulo}>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
            <div className="rounded-4xl bg-white dark:bg-slate-900/90 p-8 shadow-2xl ring-1 ring-gray-200 dark:ring-white/10 sm:p-10">
              ...
              <h2 className="mt-4 text-3xl font-bold tracking-tight sm:text-4xl">Estamos aquí para ayudarte</h2>
              <p className="mt-6 text-lg leading-8 text-gray-600 dark:text-slate-300">
    ...
    >
      ...
      {enviado ? (
        <div className="mt-8 rounded-2xl bg-green-50 dark:bg-green-900/20 p-6 text-center">
          ...
          <button type="button" onClick={() => setEnviado(false)} className="mt-4 text-sm text-pink-500 hover:text-pink-400 underline">
            Enviar otro mensaje
          </button>
        </div>
      ) : (
        <div className="mt-8 space-y-5">
          <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>
            Nombre
            <input type="text" name="nombre" value={form.nombre} onChange={(e) => setForm((p) => ({ ...p, nombre: e.target.value }))} placeholder="Tu nombre" required className={inputClass} />
          </label>
          <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>
            Correo electrónico
            <input type="email" name="correo" value={form.correo} onChange={(e) => setForm((p) => ({ ...p, correo: e.target.value }))} placeholder="tu@correo.com" required className={inputClass} />
          </label>
          <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>
            Mensaje
            <textarea name="mensaje" value={form.mensaje} onChange={(e) => setForm((p) => ({ ...p, mensaje: e.target.value }))} rows={4} placeholder="Cuéntanos cómo podemos ayudarte" required className={inputClass} />
          </label>
          <button type="submit" disabled={enviando} className="mt-3 inline-flex w-full items-center justify-center rounded-3xl bg-pink-500 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-pink-500/30 transition hover:bg-pink-400 disabled:opacity-60">
